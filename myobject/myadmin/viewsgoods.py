...
def typeindex(request,pIndex=1):
    ...
    context = {'typeslist':list2,'plist':plist, 'pIndex': pIndex}
...
def goodsindex(request):
    # 执行数据查询，并放置到模板中
    list = Goods.objects.all() #遍历商品信息表
    for ob in list:
        ty = Types.objects.get(id=ob.typeid) #获取类别表(type)中id=商品表(goods)中的typeid的商品
        ob.typename = ty.name #商品表中typename <--- 赋给type表中的name
...
def goodsinsert(request):
    try:
        ...
        # 把缩放后的图像用jpeg格式保存:
        im.save("./static/goods/s_"+filename, None) #'jpeg'
